...
# Package: ez-php/webauthn-metadata

FIDO Metadata Service (MDS3) trust-anchor lookups for WebAuthn. The one component in the
`ez-php/webauthn` family that touches the network — kept out of `ez-php/webauthn` so that
module stays "no network calls".
...
```
src/
  Aaguid.php                    — 16 raw AAGUID bytes → lowercase dashed UUID (MDS entry key)
  AttestationTrustVerifier.php  — aaguid + x5c chain → TrustDecision (the module's public entry point)
  BlobCacheInterface.php        — get()/put() of the raw JWT blob
  BlobFetcherInterface.php      — fetch(url): string — the only network seam
  CertificateChain.php          — DER chain validation (validity, signature links, anchor match) via ext-openssl
  CurlBlobFetcher.php           — https-only cURL implementation of BlobFetcherInterface
  FileBlobCache.php             — single-file cache, atomic write
  MdsClient.php                 — cache → download → verify → cache orchestration
  MetadataBlob.php              — verified payload: number, nextUpdate, entries by AAGUID
  MetadataBlobVerifier.php      — JWT verification (x5c chain to trusted root, RS256/ES256 signature) + payload parsing
  MetadataEntry.php             — one authenticator model: root certs, statuses, untrustedStatus()
  TrustDecision.php             — trusted flag, reason, matched entry
  Exception/                    — MetadataException (base), BlobFetchException, BlobVerificationException
tests/
  Support/                      — TestPki (generated certs), TestJwt (signed blobs), fakes for fetcher/cache
```
...
- **`MetadataBlobVerifier`** — the security core. Trust rests on ONE caller-supplied root certificate
  (the FIDO MDS root). The JWT `x5c` chain must lead to it with every non-leaf marked `CA:TRUE`, the
  signature must verify under the leaf key, only RS256/ES256 are accepted, and a blob past `nextUpdate`
  is rejected.
- **`MdsClient`** — returns a verified blob. Cached bytes are re-verified on every load, so a tampered
  or stale cache degrades to a re-download. A failed download propagates; a stale blob is never served.
- **`AttestationTrustVerifier`** — untrusted when the AAGUID is unlisted, when MDS reports
  REVOKED / ATTESTATION_KEY_COMPROMISE / USER_VERIFICATION_BYPASS / USER_KEY_*_COMPROMISE, or when the
  attestation chain does not lead to one of the entry's attestation root certificates.

---

## Design Decisions and Constraints

- **No dependency on `ez-php/webauthn`** (only a `suggest`). The API is plain data — AAGUID string and
  DER x5c list — so it stays standalone. Callers combine `TrustDecision::$trusted` with
  `AttestationResult::$trusted` themselves; `PublicKeyCredentialSource` does not expose x5c, so read it
  from `AttestationStatement::$attStmt['x5c']` before/while verifying registration.
- **The FIDO root is never embedded.** Shipping a root certificate in source would make a compromise or
  rotation a package release. The application passes it (PEM) to `MetadataBlobVerifier`.
- **Attestation chains are validated leniently, the blob chain strictly.** Attestation certs are issued
  by many vendors, some without proper CA constraints, so `CertificateChain` is called with
  `requireCa: false` there; the single FIDO signing chain requires CA:TRUE.
- **U2F-only entries are skipped.** MDS entries keyed by `attestationCertificateKeyIdentifiers` (no
  AAGUID) are not matched; only AAGUID-identified models are supported.
- **No revocation (CRL/OCSP) and no name/EKU policy** — signature chain, validity window and anchor only.
- **Root validity is not enforced on anchors**, only on presented chain certificates.
- **`@` on `openssl_x509_read`/`mkdir`/`rename`** is intentional: failure is handled by return value
  and turned into a decision or exception; warnings would fail callers with strict error handlers.
- `MetadataException` is the exception-hierarchy carve-out from the `final` rule.

---

## Testing Approach

- Pure PHPUnit, no external services. No network: the fetcher is faked; `CurlBlobFetcherTest` only
  covers the https guard and an unreachable local port.
- Real cryptography: `TestPki` generates EC/RSA roots, intermediates and leaves at runtime (temp openssl
  config with CA/leaf extensions) and `TestJwt` signs MDS-shaped blobs, so signature and chain paths are
  exercised for real, including ES256 R||S ↔ DER conversion over many random signatures.
- Test classes live in the shared `Tests\` namespace but must be uniquely named across the whole
  monorepo — the root `phpunit.xml` loads every package in one process, so a duplicate name is a fatal
  error, not a test failure. Prefix with `WebauthnMetadata` when the obvious name is already taken.
- The root `composer.json` `autoload-dev` `Tests\` array must list `modules/webauthn-metadata/tests/`
  (added by hand — `make_module.php --repo` does not wire it).
...
|---|---|
| Registration/assertion ceremonies, attestation *signature* verification | `ez-php/webauthn` |
| Embedding or rotating the FIDO root certificate | The consuming application |
| Generic HTTP client features (retries, proxies) | Supply your own `BlobFetcherInterface`, e.g. over `ez-php/http-client` |
| CRL/OCSP revocation checking, full X.509 path policy | Out of scope |
